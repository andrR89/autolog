...
/// Motor de sincronização para reminders.
///
...
class ReminderSyncService {
  ReminderSyncService({
    required ReminderSyncFacade facade,
    required RemoteReminderSource remote,
  }) : _facade = facade,
       _remote = remote;

  final ReminderSyncFacade _facade;
  final RemoteReminderSource _remote;
  ...
  Future<SyncResult> sync(String userId) async {
    ...
    try {
      ...
      for (final remote in remoteRows) {
        // Lê o reminder local incluindo soft-deletados, para comparar updated_at.
        final Reminder? local = await _facade.getById(remote.id);
...
final reminderSyncServiceProvider = Provider<ReminderSyncService>((ref) {
  final facade = ref.watch(reminderSyncFacadeProvider);
  final remote = ref.watch(remoteReminderSourceProvider);
  return ReminderSyncService(facade: facade, remote: remote);
});
